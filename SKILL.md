...
version: 0.1.0
description: Guided workflow for specifying, bootstrapping, implementing, adversarially auditing, testing, and shipping native Android apps (Kotlin, Jetpack Compose, Hilt) with Claude Code — distilled from sessions that produced working, shipped apps rather than from generic Android advice. Use this whenever starting a new Android app (it opens with a structured intake interview before any code), resuming/reviewing an existing one, setting up the Windows toolchain or phone/tablet/watch emulators, auditing code before a release, writing agent-executable test scenarios, processing a bug report from real hardware, or specifying a permissions/storage/cloud-sync feature. Also covers the parallel-subagent audit-then-fix workflow and the structured ADB test-harness format.
---
...
| 4 · Test scenarios | `tests/README.md` + `tests/NN-*.md` | suite runs end to end, every result PASS/FAIL/BLOCKED |
...
| Setting up a fresh Windows machine, or creating/booting phone, tablet, or Wear OS emulators | `references/windows-toolchain-and-emulators.md` |
| Auditing an app before release, fixing audit findings, processing a real-hardware bug report, writing a manual test plan | `references/testing-and-bugs.md` |
...
- **Every implementation pass ends with a real build and a real test**, not "should work now." "After running all the fixes launch the APK in the emulator... and perform a full functionality test of every feature" is a recurring, explicit instruction — say it every time rather than assuming it's implied.
