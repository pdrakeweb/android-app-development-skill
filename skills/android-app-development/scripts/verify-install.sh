...
DEVICES="$("${A[@]}" devices | awk 'NR>1 && $2=="device" {print $1}')"
...
# Otherwise a crash from a previous run reads as this run's failure.
"${A[@]}" logcat -c -b crash >/dev/null 2>&1 || true
...
# ---- did it crash? -----------------------------------------------------------
CRASH="$("${A[@]}" logcat -d -b crash 2>/dev/null | tr -d '\r')"
if [ -n "$(printf '%s' "$CRASH" | tr -d '[:space:]')" ]; then
  printf '\n%s\n' "$CRASH" | tail -40 >&2
  die "crash buffer is not empty" 4
fi
ok "crash buffer empty"
